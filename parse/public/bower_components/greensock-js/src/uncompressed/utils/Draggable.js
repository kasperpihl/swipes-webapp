			...
			Draggable = function(target, vars) {
				...
				var type = (vars.type || (_isOldIE ? "top,left" : "x,y")).toLowerCase(),
					...
					onPress = function(e) {
						...
						} else if (!(e.touches && e.touches.length > _dragCount + 1) && (e.target.nodeName + "").toUpperCase() !== "A" && !e.target.onclick) { //in some mobile browsers, e.preventDefault() when pressing on a link (or element with an onclick) will cause the link not to work.
							e.preventDefault();
						}
